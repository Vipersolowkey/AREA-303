  ...
  if (mounted && !user) {
    return (
      <div className="grid place-items-center rounded-lg border border-border bg-surface py-20 text-center">
        ...
        <p className="mt-3 text-lg font-bold">Cần đăng nhập</p>
        <p className="mt-1 max-w-sm text-text-muted">
          Đơn đặt khi chưa đăng nhập không gắn với tài khoản nào, nên không tra được ở đây —
          hãy dùng mã đơn đã hiện lúc đặt.
        </p>
        <Link
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-border pb-6">
        <h1 className="text-3xl font-extrabold">Đơn của tôi</h1>
        <p className="mt-1 text-text-muted">Lịch sử đặt hàng của tài khoản này.</p>
      </div>

      {!mounted || loading ? (
        <div className="grid place-items-center rounded-lg border border-border bg-surface py-20 text-text-muted">
          <Loader2 className="h-6 w-6 animate-spin" />
          <p className="mt-3 text-sm">Đang tải đơn…</p>
        </div>
      ) : orders === null ? (
        <div className="grid place-items-center rounded-lg border border-border bg-surface py-20 text-center">
          <PackageOpen className="h-8 w-8 text-text-dim" />
          <p className="mt-3 text-lg font-bold">Chưa tải được đơn</p>
          <p className="mt-1 text-text-muted">Kiểm tra kết nối rồi thử lại.</p>
        </div>
      ) : orders.length === 0 ? (
        <div className="grid place-items-center rounded-lg border border-border bg-surface py-20 text-center">
          <PackageOpen className="h-8 w-8 text-text-dim" />
          <p className="mt-3 text-lg font-bold">Chưa có đơn nào</p>
          <Link
            href="/shop/store"
